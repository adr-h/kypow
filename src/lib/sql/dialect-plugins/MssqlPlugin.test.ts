import { describe, it, expect } from 'vitest';
import { MssqlPlugin } from './MssqlPlugin';

describe('MssqlPlugin', () => {
  it('should correctly interpolate sql', () => {
    const rawSql = 'select * from users where id = @0 and name = @1';
    const params = [1, 'test'];
    const expected = `select * from users where id = 1 and name = 'test'`;
    const result = MssqlPlugin.interpolateSql(rawSql, params);
    expect(result).toBe(expected);
  });

  it('should handle null values', () => {
    const rawSql = 'select * from users where id = @0';
    const params = [null];
    const expected = `select * from users where id = NULL`;
    const result = MssqlPlugin.interpolateSql(rawSql, params);
    expect(result).toBe(expected);
  });

  it('should handle single quotes in string values', () => {
    const rawSql = 'select * from users where name = @0';
    const params = ["test' name"];
    const expected = `select * from users where name = 'test'' name'`;
    const result = MssqlPlugin.interpolateSql(rawSql, params);
    expect(result).toBe(expected);
  });
});
